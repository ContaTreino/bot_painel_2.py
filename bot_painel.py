...
def resolve_short(url: str) -> str:
    if "link.deezer.com" in url:
        try:
            return HTTP.head(url, allow_redirects=True, timeout=15).url
        except Exception:
            pass
    return url
...
async def _gate(event, is_cb: bool = True) -> bool:
    """
    Gate simples: o bot atende em qualquer chat (DM ou grupo) sem
    restrição de tópico. Múltiplos usuários são atendidos
    simultaneamente — cada um tem seu próprio NavState e dl_lock.
    """
...
async def _send_audio(uid: int, f: Path,
    ...
    async def _sf(chat):
        await bot.send_file(chat, str(f),
